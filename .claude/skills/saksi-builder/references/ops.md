...
- **Full transaction ledger + reputation-gaming signals**: built (per the design pass in `data-model.md`, confirmed 2026-07-20), gated behind `LEDGER_DETAIL_ENABLED`. The §4A T&C clause (drafted in that same design pass) is applied to `content/legal/syarat-ketentuan.md`. **`LEDGER_DETAIL_ENABLED=true` in Vercel production, confirmed 2026-08-01** — a deliberate call to ship without waiting for GATE 1 (lawyer review), reasoned on no paid tier collecting money yet, so the exposure this feature adds is lower-stakes than it would be once real fees are flowing. `syarat-ketentuan.md`'s own version banner still reads "DRAF, menunggu tinjauan hukum" as of this writing — that line is now inconsistent with the decision to ship without waiting, worth a look before it's read literally by a user or in any future legal review. No new migration — `lib/db/ledger.ts` is pure read-time aggregation over existing tables.
